...
class Preprocessor:
    def __init__(self, rootdir: str):
        ...
        self.re_unicode = re.compile(r'\\u[0-9a-fA-F]')
    ...
    #             for file in files:
    #                 if file.endswith(".json"):  # NOTE: If all files are guaranteed to be JSON, we can get rid of this.
    ...
    def genereate_inverted_index_from_json(self, invIndex: "InvertedIndex") -> None:
        """
        Similar to load_data() (deprecated);
            differ that it calls html_parser and tokenize right away whilst reading file
        """
        docNo = 0
        #data: Dict[str, Dict[str, str]] = {}  # {filepath: {data type (e.g. url, html content): corresponding data value}}
        try:
            for root, _, files in os.walk(self.root):  # os.walk returns [current folder, subfolders, files] and if subfolders exists, it 
                for file in files:
                    ...
                    pure_text = self.parse_html(content)
                    token_list = self.tokenize(pure_text)

                    for tok in token_list:
                        invIndex.add(tok, (docNo, 1))
    ...
    def tokenize(self, text: str) -> List[str]:
        ...
        #words = self.re_alnum.split(r'[^a-z0-9]+', text)  # Alternatively use split instead of findall
        text = self.re_alnum.sub(' ', text)
    ...
    def getFileCounts(self) -> int:
        return self.parsedFilesCount
